...
// (Support / Oppose / Feature for views; Feature only for other types).
// If a pin already exists, the form is pre-filled with the current kind.
func (s *Server) handlePinForm(w http.ResponseWriter, r *http.Request) {
	...
	render(w, r, views.PinForm(viewerFor(user), node, "", initialPinForm(node, current)))
}
